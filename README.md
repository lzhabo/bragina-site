...
## ⚠️ Если деплой падает с ошибкой `cp: cannot stat 'styles.css'`

Это значит, что в репозитории остался **старый/чужой сценарий сборки**, который
вручную копирует несуществующий файл `styles.css`. Наш сайт использует
`tm-styles.css`, поэтому сборка падает. Исправляется одним из двух способов:

**Способ 1 — самый простой (без кода):**
В **Settings → Pages → Build and deployment → Source** выбери
**Deploy from a branch** (вместо «GitHub Actions»), ветку `main`, папку `/ (root)`,
нажми **Save**. GitHub будет отдавать файлы как есть, без сборки — ошибки не будет.

**Способ 2 — оставить «GitHub Actions»:**
В этой папке уже лежит правильный сценарий `.github/workflows/deploy.yml`,
который грузит всю папку целиком. Залей папку заново вместе с ним и **удали
старый сломанный workflow** (любой другой `.yml` в `.github/workflows/`,
где есть строка `cp ... styles.css`).
